document.addEventListener("DOMContentLoaded", function (){
  ...
  const searchInput:HTMLInputElement | null = document.querySelector(".search-bar input");
  ...
  interface Books {
    find(arg0: (book: any) => boolean): unknown;
    title: string;
    author: string;
    description: string;
    genre: string;
  ...
  function filterAndSortBooks() {
    ...
    let filteredBooks:Book[] = allBooks.filter(book => {
  ...
  function updateStats(books: Book[]) {
